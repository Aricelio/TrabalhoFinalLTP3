        ...
        if (obj.getCodigo() == 0) {
            try {
                PreparedStatement sqlInsert = getConexao().prepareStatement
                        ("insert into Refeicao(preco,descricao,codTipoRefeicao,ativo) values(?,?,?,?)");
                ...
                PreparedStatement sqlConsulta = getConexao().prepareStatement
                        ("select codRefeicao from Refeicao where descricao = ? and preco=? and codTipoRefeicao=?");
            ...
            try {
                ...
                PreparedStatement sql = con.prepareStatement
                        ("update Refeicao set Descricao=? , preco=? , codTipoRefeicao=? where codRefeicao=?");
        ...
        if ((obj.getCodigo() > 0) && (obj.getAtivo() == 1)) {
            try {
                ...
                PreparedStatement sqlUpdate = getConexao().prepareStatement
                        ("update Refeicao set ativo = 0 where codRefeicao=?");
    ...
    public Refeicao Abrir(int id) {
        try {
            PreparedStatement sql = getConexao().prepareStatement
                    ("select * from refeicao where codRefeicao=? and ativo = 1");
            sql.setInt(1, id);
